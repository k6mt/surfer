package com.github.k6mt.surfer.analysis.core;
...
public class MethodFlowAnalysisService {
  ...
  private String buildAnalysisPrompt(Trace trace) {
    ...
    prompt.append(
        "Please analyze the method execution flow and source code below. Identify performance issues, potential bugs, design patterns, and suggest improvements.\n\n");
    prompt.append("Please provide your analysis in Korean language only.");

    // Add method source code
    prompt.append("# Source Code\n");
    String rootMethodSource = sourceCapture.captureMethodSource(trace.getClassName(),
        trace.getMethodName());
  ...
  private void addNestedTraces(StringBuilder prompt, List<Trace> traces, int depth) {
    ...
    for (Trace trace : traces) {
      ...
      prompt.append(indent).append("  - Execution Time: ").append(trace.getResultTimeMs())
          .append("ms\n");

      if (trace.getExceptionMessage() != null) {
        prompt.append(indent).append("  - Exception Occurred: ").append(trace.getExceptionMessage())
            .append("\n");
      }
